...
namespace QuanLyDeTai.BoMon
{
    public partial class GV : Form
    {
...

        {
            ...
            string query = "select * from GV where MABM = '" + MaBM + "' and TenGV like '%"+tb_tk.Text.Trim()+"%'";
            DataTable dt = ConnectDB.Connected.getData(query);
            listgv.DataSource = dt;
        }
        ...
        private void listgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try {
                ...
                string MAGV = dataGridViewRow.Cells["MAGV"].Value.ToString();
                string TenGV = dataGridViewRow.Cells["TenGV"].Value.ToString();
                string NS = dataGridViewRow.Cells["NS"].Value.ToString().Split()[0];
                string GioiTinh = dataGridViewRow.Cells["GioiTinh"].Value.ToString();
                string CapBac = dataGridViewRow.Cells["CapBac"].Value.ToString();
                string ChucVu = dataGridViewRow.Cells["ChucVu"].Value.ToString();
                string DTL = dataGridViewRow.Cells["DTL"].Value.ToString();
                string MABM = dataGridViewRow.Cells["MABM"].Value.ToString();
